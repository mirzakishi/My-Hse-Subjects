setwd("/Users/Public/My-Hse-Subjects/Econometrics with R/Coursera/Week 8/") # Set directory
...
# то же случайное блуждание, только 501 наблюдение(Random walk)
...
prognoz <- forecast::forecast(mod_2, h=5)
prognoz
...
prognoz_4 <- forecast::forecast(mod_4, h = 5)
plot(prognoz_4)
...
prognoz_a <- forecast::forecast(mod_a, h = 5)
plot(prognoz_a)
...
getSymbols(Symbols = "GOOG", from = "2014-01-01", to = "2016-05-11")
...
prognoz_1 <- forecast::forecast(mod_1, h = 20)
prognoz_1
...
prognoz_1 <- forecast::forecast(mod_1, h = 5)
plot(prognoz_1)

# индекс потребительских цен (ежемесячные данные)
y <- sophisthse("CPI_M_CHI")
y
write.csv(sophisthse("CPI_M_CHI"), file="CPI.csv")
plot(y)
tsdisplay(as.ts(y))
...
prognoz_1 <- forecast::forecast(mod_1, h = 12)
summary(prognoz_1)
plot(prognoz_1)
...
# прогнозируем на год вперед
prognoz_a <- forecast::forecast(mod_a, h = 12)
plot(prognoz_a)
