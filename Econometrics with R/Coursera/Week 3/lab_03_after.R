...
bg <- qplot(data = h, log(carat), log(price))+ stat_smooth(method = "lm")
bg
...
f <- read_excel("flats_moscow.xlsx")
...
qplot(data = f, log(price), geom="density")
...
summary(model_2)  # базовый вариант отчета о модели
...
n <- 21.9+80^2*0.01+2*80*(-0.46)+1259.265
n
